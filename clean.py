...
def delete_generations(generations, prefix=RIA_MODEL_SNAPSHOTS_PREFIX, test=False):
    if len(generations) == 0:
        ...
        if len(generations) == 1:
            ...
            indexes = [i for i in indexes if i >= generations[0] and i <= generations[1]]
        files = []
        for i in indexes:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--generations', nargs='*', action='store', type=int, help='range for generations as "FIRST [LAST]"')
